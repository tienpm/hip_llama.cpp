...
void copy_weight_pipeline_to_host_batch_layer_swap(thablasHandle_t handle, Transformer* t_h, TransformerWeights* &w_h, int pipe_size, int pipe_id, int batch_size)
{
  ...
  w_h = (TransformerWeights*)malloc(sizeof(TransformerWeights));
  // Config config; // the hyperparameters of the architecture (the blueprint)
  // TransformerWeights weights; // the weights of the model
  // RunState state; // buffers for the "wave" of activations in the forward pass
  // // some more state needed to properly clean up the memory mapping (sigh)
  ...
  // CHECK_HIP(hipHostMalloc(&w_h->token_embedding_table, vocab_size * dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->rms_att_weight, pipe_size * dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->rms_ffn_weight, pipe_size * dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->wq, pipe_size * dim * dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->wk, pipe_size * dim * kv_dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->wv, pipe_size * dim * kv_dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->wo, pipe_size * dim * dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->w1, pipe_size * hidden_dim * dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->w2, pipe_size * dim * hidden_dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->w3, pipe_size * hidden_dim * dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->rms_final_weight, dim * sizeof(float)));
  // CHECK_HIP(hipHostMalloc(&w_h->wcls, dim * vocab_size * sizeof(float)));  

  w_h->token_embedding_table = (float*)malloc(vocab_size * dim * sizeof(float));
  w_h->rms_att_weight        = (float*)malloc(pipe_size * dim * sizeof(float));
  w_h->rms_ffn_weight        = (float*)malloc(pipe_size * dim * sizeof(float));
  w_h->wq                    = (float*)malloc(pipe_size * dim * dim * sizeof(float));
  w_h->wk                    = (float*)malloc(pipe_size * dim * kv_dim * sizeof(float));
  w_h->wv                    = (float*)malloc(pipe_size * dim * kv_dim * sizeof(float));
  w_h->wo                    = (float*)malloc(pipe_size * dim * dim * sizeof(float));
  w_h->w1                    = (float*)malloc(pipe_size * hidden_dim * dim * sizeof(float));
  w_h->w2                    = (float*)malloc(pipe_size * dim * hidden_dim * sizeof(float));
  w_h->w3                    = (float*)malloc(pipe_size * hidden_dim * dim * sizeof(float));
  w_h->rms_final_weight      = (float*)malloc(dim * sizeof(float));
  w_h->wcls                  = (float*)malloc(dim * vocab_size * sizeof(float));

  int poff = pipe_id * pipe_size;
  memcpy(w_h->token_embedding_table, t_h->weights.token_embedding_table, vocab_size * dim * sizeof(float));
  memcpy(w_h->rms_att_weight,        t_h->weights.rms_att_weight + poff * dim,  pipe_size * dim * sizeof(float));
  memcpy(w_h->rms_ffn_weight,        t_h->weights.rms_ffn_weight + poff * dim,  pipe_size * dim * sizeof(float));
  memcpy(w_h->wq,                    t_h->weights.wq + poff * dim * dim,        pipe_size * dim * dim * sizeof(float));
  memcpy(w_h->wk,                    t_h->weights.wk + poff * dim * kv_dim,     pipe_size * dim * kv_dim * sizeof(float));
  memcpy(w_h->wv,                    t_h->weights.wv + poff * dim * kv_dim,     pipe_size * dim * kv_dim * sizeof(float));
  memcpy(w_h->wo,                    t_h->weights.wo + poff * dim * dim,        pipe_size * dim * dim * sizeof(float));
  memcpy(w_h->w1,                    t_h->weights.w1 + poff * hidden_dim * dim, pipe_size * hidden_dim * dim * sizeof(float));
  memcpy(w_h->w2,                    t_h->weights.w2 + poff * dim * hidden_dim, pipe_size * dim * hidden_dim * sizeof(float));
  memcpy(w_h->w3,                    t_h->weights.w3 + poff * hidden_dim * dim, pipe_size * hidden_dim * dim * sizeof(float));
  memcpy(w_h->rms_final_weight, t_h->weights.rms_final_weight, dim * sizeof(float));
  memcpy(w_h->wcls, t_h->weights.wcls, dim * vocab_size * sizeof(float));
}
...
void alloc_run_state_to_host_batch(thablasHandle_t handle, Transformer* t_h, RunState* &s_h, int pipe_size, int pipe_id, int batch_size)
{
  ...
  // CHECK_HIP(hipHostMalloc(&s_h->att, n_heads * seq_len * sizeof(float) * batch_size));
  // CHECK_HIP(hipHostMalloc(&s_h->logits, vocab_size * sizeof(float) * batch_size));
  // CHECK_HIP(hipHostMalloc(&s_h->key_cache, pipe_size * seq_len * kv_dim * sizeof(float) * batch_size));
  // CHECK_HIP(hipHostMalloc(&s_h->value_cache, pipe_size * seq_len * kv_dim * sizeof(float) * batch_size));
  s_h->x           = (float*)malloc(dim * sizeof(float) * batch_size);
  s_h->xb          = (float*)malloc(dim * sizeof(float) * batch_size);
  s_h->xb2         = (float*)malloc(dim * sizeof(float) * batch_size);
  s_h->hb          = (float*)malloc(hidden_dim * sizeof(float) * batch_size);
  s_h->hb2         = (float*)malloc(hidden_dim * sizeof(float) * batch_size);
  s_h->q           = (float*)malloc(dim * sizeof(float) * batch_size);
  s_h->att         = (float*)malloc(n_heads * seq_len * sizeof(float) * batch_size);
  s_h->logits      = (float*)malloc(vocab_size * sizeof(float) * batch_size);
  s_h->key_cache   = (float*)malloc(pipe_size * seq_len * kv_dim * sizeof(float) * batch_size);
  s_h->value_cache = (float*)malloc(pipe_size * seq_len * kv_dim * sizeof(float) * batch_size);
}
