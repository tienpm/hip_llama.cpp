...
thablasStatus_t thaDNN_s_forward_batch_multiple_pipe_line_cache_swap(thablasHandle_t handle[], int fid, int n_flows, int n_devices, int batch_size, int n_cache_words, Config* p, TransformerWeights* w[], RunState* s_batch[], RunState* s_host_batch[], int token[], int pos[], float* logits_host, int* flow_status, int* device_flow, std::mutex *device_mtx) {
    ...
    for(int gid=0; gid<n_devices ; ++gid) { // loop each device 
        ...
        for(unsigned long long l = 0; l < pipe_size; l++) {
            ...
            {
                // copy data from swap memory to device
                int n_swap_words = p->seq_len - n_cache_words;
                int n_words_to_swap = max_pos + 1 - n_cache_words;
                int cached_words = n_cache_words * batch_size * kv_dim;

                s_batch_key_layer_cache = s_batch[gid]->key_cache_swap;
                s_batch_value_layer_cache = s_batch[gid]->value_cache_swap;
                CHECK_HIP(hipMemcpyAsync(s_batch_key_layer_cache, s_batch[gid]->key_cache + loff, cached_words * sizeof(float), hipMemcpyDeviceToDevice, handle[gid].copy_stream));
                CHECK_HIP(hipMemcpyAsync(s_batch_value_layer_cache, s_batch[gid]->value_cache + loff, cached_words * sizeof(float), hipMemcpyDeviceToDevice, handle[gid].copy_stream));

                int host_loff = l * n_swap_words * batch_size * kv_dim;
                CHECK_HIP(hipMemcpyAsync(s_batch_key_layer_cache + cached_words, s_host_batch[gid]->key_cache + host_loff, n_words_to_swap * batch_size * kv_dim * sizeof(float), hipMemcpyHostToDevice, handle[gid].copy_stream));
            ...
            if (max_pos + 1 > n_cache_words) 
            {
                ...
                CHECK_HIP(hipMemcpyAsync(s_batch[gid]->key_cache + loff, s_batch_key_layer_cache, cached_words * sizeof(float), hipMemcpyDeviceToDevice, handle[gid].copy_stream));
                CHECK_HIP(hipMemcpyAsync(s_batch[gid]->value_cache + loff, s_batch_value_layer_cache, cached_words * sizeof(float), hipMemcpyDeviceToDevice, handle[gid].copy_stream));
